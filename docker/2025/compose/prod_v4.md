...
services:
  desa-plus-proxy:
    ...
    container_name: desa-plus-proxy
  ...
  desa-plus-postgres:
    image: postgres:16
    container_name: desa-plus-postgres
    restart: unless-stopped
    environment:
      - POSTGRES_USER=bip
      - POSTGRES_PASSWORD=Production_123
      - POSTGRES_DB=desa-plus
    volumes:
      - ./data/postgres:/var/lib/postgresql/data
    networks:
      - desa-plus
    healthcheck:
      test: ['CMD-SHELL', 'pg_isready -U bip -d desa-plus']
      interval: 5s
      timeout: 5s
      retries: 5
  desa-plus-frpc:
    image: snowdreamtech/frpc:latest
    container_name: desa-plus-frpc
    restart: always
    volumes:
